...
const router = useRouter();
  ...
  return (
    ...
    {Object.entries(requests).map(([key, {title, url}]) => 
       ( <h2 key={key} onClick={() => router.push(`/?genre=${key}`)} className={`font-sans font-semibold  flex-none select-none text-lg m-2 last:pr-24 cursor-pointer transition snap-centre duration-100 hover:scale-110 hover:text-white active:text-red-500 ${
                  router.asPath === title
                    ? "text-primary-500"
                    : "text-secondary-300"
                }`}>{title}
